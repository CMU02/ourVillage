...
interface AlarmInfo {
    type: "폭염주의보" | "폭염경보" | "호우주의보" | "호우경보" | "강풍주의보" | "강풍경보" | "낙뢰주의보" | "한파주의보" | "한파경보";
    iconPath: string;
    color: string;
    message: string;
}
...
export default function WeatherAlarm({ weatherInfo }: WeatherAlarmProps) {
    ...
    const getWeatherData = () => {
        ...
        return {
            temperature: t1hItems.length ? parseFloat(getClosestItem(t1hItems)?.fcstValue || "0") : 0,
            rainfall: rn1Items.length ? parseFloat(getClosestItem(rn1Items)?.fcstValue || "0") : 0,
            windSpeed: wsdItems.length ? parseFloat(getClosestItem(wsdItems)?.fcstValue || "0") : 0,
            lightning: lgtItems.length ? parseFloat(getClosestItem(lgtItems)?.fcstValue || "0") : 0,
        };
    };
